...
import com.example.bharatbuzz.databinding.FragmentTechnologyBinding;
...
public class TechnologyFragment extends Fragment implements HomeFragment.SearchableFragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        binding.swipeRefreshLayout.setOnRefreshListener(this::FetchTechnologyNews);
    ...
    private void setUpAdapters() {
        ...
        binding.viewPagerTopTechNews.setAdapter(topAdapter);

        latestAdapter = new NewsAdapter(requireContext(), latestNewsList);
        binding.rvTechNews.setLayoutManager(new LinearLayoutManager(requireContext()));
        binding.rvTechNews.setAdapter(latestAdapter);
    }

    private void FetchTechnologyNews() {
        ApiClient.getApiInterface().getCategoryNews("us", "technology", 1, 50, API_KEY)
        ...
        autoSwipeRunnable = new Runnable() {
            ...
            public void run() {
                if (binding != null && topNewsList.size() > 0) {
                    int currentItem = binding.viewPagerTopTechNews.getCurrentItem();
                    int nextItem = (currentItem + 1) % topNewsList.size();
                    binding.viewPagerTopTechNews.setCurrentItem(nextItem, true);
    ...
    public void onSearchQuery(String query) {
        if (query == null || query.isEmpty()) {
            ...
            binding.textTopTechHeadlines.setVisibility(View.VISIBLE);
            binding.viewPagerTopTechNews.setVisibility(View.VISIBLE);
        } else {
            binding.textTopTechHeadlines.setVisibility(View.GONE);
            binding.viewPagerTopTechNews.setVisibility(View.GONE);
