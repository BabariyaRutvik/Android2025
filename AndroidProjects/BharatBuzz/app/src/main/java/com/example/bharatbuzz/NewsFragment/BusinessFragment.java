...
import com.example.bharatbuzz.databinding.FragmentBusinessBinding;
...
public class BusinessFragment extends Fragment implements HomeFragment.SearchableFragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        binding.swipeRefreshLayout.setOnRefreshListener(this::fetchBusinessNews);
    ...
    private void setupAdapters() {
        ...
        binding.viewPagerTopBusinessNews.setAdapter(topAdapter);

        latestAdapter = new NewsAdapter(requireContext(), latestNewsList);
        binding.rvBusinessNews.setLayoutManager(new LinearLayoutManager(requireContext()));
        binding.rvBusinessNews.setAdapter(latestAdapter);
    }

    private void fetchBusinessNews() {
        ApiClient.getApiInterface().getCategoryNews("us", "business", 1, 50, API_KEY)
        ...
        autoSwipeRunnable = new Runnable() {
            ...
            public void run() {
                if (binding != null && topNewsList.size() > 0) {
                    int currentItem = binding.viewPagerTopBusinessNews.getCurrentItem();
                    int nextItem = (currentItem + 1) % topNewsList.size();
                    binding.viewPagerTopBusinessNews.setCurrentItem(nextItem, true);
    ...
    public void onSearchQuery(String query) {
        if (query == null || query.isEmpty()) {
            ...
            binding.textTopBusinessHeadlines.setVisibility(View.VISIBLE);
            binding.viewPagerTopBusinessNews.setVisibility(View.VISIBLE);
        } else {
            binding.textTopBusinessHeadlines.setVisibility(View.GONE);
            binding.viewPagerTopBusinessNews.setVisibility(View.GONE);
